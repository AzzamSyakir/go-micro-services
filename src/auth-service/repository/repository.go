...
func (userRepository *UserRepository) CreateUser(begin *sql.Tx, toCreateUser *entity.Auth) (result *entity.Auth, err error) {
...
func (userRepository *UserRepository) FetchUser(begin *sql.Tx) (result *model_response.Response[[]*entity.Auth], err error) {
	var rows *sql.Rows
	var queryErr error
	...
	defer rows.Close()
	var fetchUsers []*entity.Auth
	for rows.Next() {
		fetchUser := &entity.Auth{}
		scanErr := rows.Scan(
			&fetchUser.Id,
			&fetchUser.Name,
			&fetchUser.Email,
...
func DeserializeUserRows(rows *sql.Rows) []*entity.Auth {
	var foundUsers []*entity.Auth
	for rows.Next() {
		foundUser := &entity.Auth{}
		scanErr := rows.Scan(
			&foundUser.Id,
			&foundUser.Name,
			&foundUser.Email,
...
func (userRepository *UserRepository) GetOneById(begin *sql.Tx, id string) (result *entity.Auth, err error) {
	var rows *sql.Rows
	var queryErr error
	rows, queryErr = begin.Query(
...
func (userRepository *UserRepository) PatchOneById(begin *sql.Tx, id string, toPatchUser *entity.Auth) (result *entity.Auth, err error) {
	rows, queryErr := begin.Query(
...
func (userRepository *UserRepository) DeleteUser(begin *sql.Tx, id string) (result *entity.Auth, err error) {
	rows, queryErr := begin.Query(
